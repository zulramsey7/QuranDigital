    ...
    <MainLayout>
      <div className="min-h-screen space-y-6 animate-fade-in pb-32 max-w-md mx-auto px-4 relative">
        ...
        <div className="flex items-center justify-between pt-6">
          ...
          <div className="flex gap-2">
             ...
             }} className="px-3 h-10 rounded-2xl bg-secondary/50 flex items-center gap-2 text-[10px] font-black dark:text-white uppercase border border-white/5 active:scale-95 transition-all">
               <Gauge className="w-3 h-3 text-primary" /> {playbackSpeed}x
             </button>

             <button onClick={() => { setShowVolumeMenu(!showVolumeMenu); setShowTimerMenu(false); }} className={cn("w-10 h-10 rounded-2xl flex items-center justify-center transition-all bg-secondary/50 active:scale-95", showVolumeMenu && "bg-primary text-black")}>
               {isMuted || volume === 0 ? <VolumeX className="w-4 h-4" /> : <Volume2 className="w-4 h-4" />}
             </button>

             <button onClick={() => { setShowTimerMenu(!showTimerMenu); setShowVolumeMenu(false); }} className={cn("w-10 h-10 rounded-2xl flex items-center justify-center transition-all active:scale-95", timeLeft ? "bg-primary text-black" : "bg-secondary/50")}>
        ...
        {/* Timer Popover */}
        {showTimerMenu && (
          <div className="absolute top-20 right-4 bg-white dark:bg-slate-900 border border-black/5 shadow-2xl rounded-[24px] p-2 z-50 w-32 animate-in slide-in-from-top-2">
            {[15, 30, 60].map(mins => (
        ...
        {/* Player Card */}
        <div className={cn(
          "relative overflow-hidden rounded-[32px] p-8 shadow-2xl transition-all duration-700", 
          activeTab === 'nasyid' ? "bg-slate-950" : 
          activeTab === 'sholawat' ? "bg-emerald-950" : 
          activeTab === 'zikir' ? "bg-indigo-950" : "bg-rose-950"
        )}>
          <div className="relative z-10 flex flex-col items-center text-center space-y-6">
            <div className={cn("w-28 h-28 rounded-[32px] flex items-center justify-center backdrop-blur-3xl border transition-all duration-700", 
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        input[type='range']:not(.volume-slider-vertical)::-webkit-slider-thumb {
          ...
          cursor: pointer; box-shadow: 0 0 10px rgba(16, 185, 129, 0.8);
          margin-top: -6px;
        }
