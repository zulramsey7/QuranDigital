...
async function fetchSurahList(): Promise<Surah[]> {
  ...
  try {
    ...
    console.error('Failed to cache surah list', e);
  }
...
async function fetchSurahAyahs(surahNumber: number): Promise<Ayah[]> {
  const CACHE_KEY = `quran_surah_${surahNumber}`;
  const cached = localStorage.getItem(CACHE_KEY);

  if (cached) {
    try {
      // Data surah tak berubah, so no expiry check needed really, 
      // but let's put safe guard if schema changes
      return JSON.parse(cached);
    } catch (e) {
      console.error('Error reading ayah cache', e);
    }
  }
  ...
  const formattedAyahs = data.data.ayat.map((v) => ({
  ...
  try {
    localStorage.setItem(CACHE_KEY, JSON.stringify(formattedAyahs));
  } catch (e) {
    // If quota exceeded, maybe clear old caches? For now just log
    console.error('Failed to cache surah ayahs', e);
  }
