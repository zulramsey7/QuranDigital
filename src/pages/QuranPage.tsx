...
export default function QuranPage() {
  ...
  const fetchSurahDetails = async (surahNumber: number) => {
    ...
    try {
      const [resUthmani, resLatin] = await Promise.all([
        fetch(`https://api.quran.com/api/v4/verses/by_chapter/${surahNumber}?language=ms&words=false&translations=39&fields=text_uthmani`),
  ...
  // VIEW: DETAIL SURAH (Jika Surah Dipilih)
  if (selectedSurah) {
    ...
    return (
      <MainLayout>
        ...
        <div className="space-y-6 animate-fade-in pb-20">
          {/* Header with Back Button Style Sirah.tsx */}
  ...
  // VIEW: LIST SURAH (Halaman Utama)
  return (
    <MainLayout>
      <div className="space-y-6 animate-fade-in pb-20">
        {/* Header Back Button Style Sirah.tsx */}
        <div className="flex items-center gap-4">
          <button 
            onClick={() => navigate('/')}
            className="w-10 h-10 rounded-2xl bg-white dark:bg-slate-800 shadow-sm border border-black/5 flex items-center justify-center hover:bg-secondary transition-all active:scale-95"
          ...
          <div className="text-left">
            <h1 className="text-2xl font-bold tracking-tight">{t('quran')}</h1>
