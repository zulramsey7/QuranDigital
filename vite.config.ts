    ...
    VitePWA({
      ...
      workbox: {
        ...
        runtimeCaching: [
          ...
          {
            urlPattern: /^https:\/\/api\.waktusolat\.app\/.*/i,
            handler: "CacheFirst",
            options: {
              cacheName: "prayer-times-cache-v2",
              expiration: {
                maxEntries: 15, // Simpan data untuk 15 zon berbeza
                maxAgeSeconds: 60 * 60 * 24 * 30, // Simpan sebulan (API v2 return setahun data pun ok, tapi kita cache sebulan cukup)
              },
              cacheableResponse: {
                statuses: [0, 200],
              },
          ...
          },
          {
            ...
            handler: "CacheFirst",
            options: {
              cacheName: "quran-ayah-cache",
              expiration: {
                maxEntries: 114, // Simpan semua 114 surah jika user buka semua
                maxAgeSeconds: 60 * 60 * 24 * 365, // Setahun (teks Quran tak berubah)
              },
              cacheableResponse: {
                statuses: [0, 200],
              },
